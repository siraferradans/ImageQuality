...
J=5;
   ...
   disp(num2str(J))
    ...
    regressiontype=RegressionType{3};
    
    vLambda = [0.001 0.01 0.1 1 10 100 0.0001 1e-5 1e-8 0];
    for ivr=7:length(vLambda)
...
save('./errsLambdas_newerror.mat','err_scat','options','vLambda');
h=figure;Js = log10(vLambda+eps);

[v,indx]=sort(Js);
%plot error to get the best lambda
h=figure;plot(Js(indx),err_scat(indx,1))
xlabel('\lambda');title('J=5, rgb')
saveas(h,'./lambda_vs_err.png')
h=figure;
